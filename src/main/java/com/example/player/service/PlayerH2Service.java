...
public class PlayerH2Service implements PlayerRepository{
    ...
    public ArrayList<Player> getPlayers(){
        List<Player> playersall = db.query("select * from team" , new PlayerRowMapper());
        ArrayList<Player> players = new ArrayList<>(playersall);
        return players;
    }
    @Override
    ...
    public Player addPlayer(Player player){
        ...
        Player savedBook = db.queryForObject("select * from team where playerName = ? and jerseyNumber = ? and role = ?",new PlayerRowMapper(),player.getPlayerName(),player.getJerseyNumber(),player.getRole());
        return savedBook;
